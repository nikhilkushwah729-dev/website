...
app.post("/api/ai/proposal", async (req, res) => {
  try {
    ...
    if (!ai) {
      // Fallback rich proposal if API key is not configured yet
      return res.json({
        success: true,
        isFallback: true,
        proposal: {
          ...
          phases: [
            {
              ...
              duration: "1 - 2 Weeks",
              deliverables: ["Interactive Figma Wireframes", "Database Schema Design", "Component System & Theme Setup"]
            },
            {
              phase: "Phase 2: Core Engineering & Integrations",
              duration: "2 - 4 Weeks",
              deliverables: ["Frontend & Backend Development", "API & Payment Integrations", "Real-time State Management"]
            },
            {
              phase: "Phase 3: QA, Performance & Launch",
              duration: "1 Week",
              deliverables: ["Lighthouse 95+ Audit", "Security & Load Testing", "Deployment & Handover Docs"]
            }
          ],
...
`;
    ...
    console.error("AI Proposal Error:", error?.message || error);
    // Graceful fallback
    return res.json({
      success: true,
      isFallback: true,
      errorNotice: "Generated using standard client template",
      proposal: {
...
// AI Client Assistant Chat Endpoint
...
Nexus offers:
- Flagship Software Product Licenses ($29 - $1,299/mo)
- Custom Full-Stack Enterprise Solutions ($3k - $15k+)
- AI & Cloud Workflow Architectures ($2.5k - $8k)
- Technical Advisory & Code Audits

Keep responses concise (2-4 short sentences), encouraging the client to explore the Product Sandbox, test the API Hub, or click "Request License" / "Request Technical Proposal".
`;
